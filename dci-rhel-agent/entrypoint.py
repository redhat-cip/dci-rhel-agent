...
def main():
  with open('/etc/dci-rhel-agent/settings.yml', 'r') as settings:
    ...
    if 'systems' in extravars.keys():
      ...
      print ('No hosts found in settings ! Trying to use all systems available in Beaker.')
      try:
        p = subprocess.Popen("bkr system-list --free", stdout=subprocess.PIPE, shell=True)
