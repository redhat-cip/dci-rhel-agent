...
def main():
  ...
  if environ.get('DCI_LOCAL_REPO') is not None:
    local_repo = os.getenv("DCI_LOCAL_REPO")
  else:
    print ("Error ! Environmental variable DCI_LOCAL_REPO not set.")
    sys.exit(1)
  ...
  if t.status_code != 200:
    ...
    sys.exit(1)
  topic_id = topic_id.json()["topics"][0]['id']
  ...
  if 'systems' in extravars.keys():
    ...
    print ('Error ! No hosts (systems) found in settings.')
    sys.exit(1)
