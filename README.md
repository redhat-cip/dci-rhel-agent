...
| job_xml.path | False | String | Path to a custom XML file to use with Beaker job. |
...
#### How to use a custom Beaker XML job file tags to a job ?

If you want to use your own XML file to submit to `bkr-job-submit (1)`, you can use in `settings.yml` the property `job_xml['path']`:

```
job_xml:
  path: /path/to/job.xml
```
