...
RUN yum upgrade -y && \
  ...
  yum --nogpgcheck -y install gcc ansible python python2-devel python2-pip beaker-client beaker-lab-controller ansible-role-dci-import-keys ansible-role-dci-retrieve-component dci-ansible ansible-role-dci-rhel-certification && \
  yum clean all
