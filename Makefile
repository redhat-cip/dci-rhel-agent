...
run81:
    ...
    LOCAL_STORAGE_FOLDER=$$DCI_LOCAL_REPO /bin/dci-downloader --topic "RHEL-8.1" --arch "x86_64" --arch "ppc64le" && \
    podman pull quay.io/distributedci/dci-rhel-agent:stable && podman run --rm -ti --network host \
	-e DCI_CLIENT_ID \
	-e DCI_API_SECRET \
	-e DCI_CS_URL \
	-e DCI_LOCAL_REPO \
